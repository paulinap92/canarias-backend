...
@router.get("/beaches")
async def live_beaches(island:str|None=Query(None),hours:int=Query(48,ge=12,le=96))->dict[str,Any]: return src("beaches").read(island=island,hours=hours)
@router.post("/beaches")
async def refresh_live_beaches(island:str|None=Query(None),hours:int=Query(48,ge=12,le=96))->dict[str,Any]: return await src("beaches").refresh(island=island,hours=hours)
